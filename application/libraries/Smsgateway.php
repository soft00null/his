...
class Smsgateway {
    ...
    function sentDischargedSMS($id) {
        $sms_detail = $this->_CI->smsconfig_model->getActiveSMS();

        $data = $this->getPatientDischargedContent($id);
        $msg = $data["msg"];
    ...
    public function getPatientDischargedContent($id) {
        $session_name = $this->_CI->setting_model->getCurrentSessionName();
        $patient = $this->_CI->patient_model->patientProfile($id, 'no');
        $hospital_details = $this->_CI->setting_model->getHospitalDetails();
        $currency_symbol = $this->sch_setting[0]['currency_symbol'];
        $hospital_name = $hospital_details["name"];
        $msg = $patient['patient_name'] .
                " is now discharged from " . $hospital_name . " . Your final bill amount was " . $currency_symbol . $patient["net_amount"];
